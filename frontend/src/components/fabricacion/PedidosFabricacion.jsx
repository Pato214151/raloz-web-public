...
function ModalSaldo({ pedido, onClose, onDone }) {
  ...
  const confirmar = async () => {
    ...
    try {
      const res = await api.post(`/tienda/admin/fabricacion/pedidos/${pedido.id_pedido}/registrar-saldo`, { monto: val, metodo })
      onDone(res.data.pedido)
    } catch (err) {
...
function PedidoCard({ pedido: initialPed, onRefresh }) {
  ...
  const call = async (ruta) => {
    setAccion(ruta)
    try {
      const res = await api.post(`/tienda/admin/fabricacion/pedidos/${ped.id_pedido}/${ruta}`)
      if (res.data.pedido) setPed(res.data.pedido)
      else onRefresh()
    } catch {
  ...
  const guardarFecha = async () => {
    ...
    setAccion('fecha')
    try {
      await api.post(`/tienda/admin/fabricacion/pedidos/${ped.id_pedido}/actualizar-fecha`, { fecha_estimada: fechaEdit })
  ...
  const notificarWhatsApp = async () => {
    ...
    try {
      const res = await api.post(`/tienda/admin/fabricacion/pedidos/${ped.id_pedido}/marcar-notificado`)
      if (res.data.pedido) setPed(res.data.pedido)
    } catch { /* no crítico */ }
  }
    ...
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 mt-0.5">{ped.nombre_colegio} · {ped.fecha_pedido?.slice(0, 10)}</p>
        </div>
        ...
        <div className="border-t border-gray-100 px-4 pb-4 pt-3 space-y-4">
                ...
                <tbody className="divide-y divide-gray-50">
                  {(ped.items || []).map((item, i) => (
                    <tr key={i} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-3 py-2 text-center"><BadgeTipo tipo={item.tipo_pedido} /></td>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 flex items-center gap-2 flex-wrap">
              ...
              {editFecha ? (
                <>
                  <input type="date" value={fechaEdit} onChange={e => setFecha(e.target.value)}
                ...
                <>
                  ...
                  {ped.estado !== 'entregado' && (
                    <button onClick={() => setEdit(true)} className="text-xs text-amber-600 hover:text-amber-700 hover:underline">
                      Editar
                    </button>
                  )}
                </>
          ...
          {ped.estado !== 'entregado' && (
            <div className="flex flex-wrap gap-2 pt-1 border-t border-gray-100">
              {ped.estado === 'en_produccion' && (
                <button onClick={() => call('marcar-listo')} disabled={accion === 'marcar-listo'}
              ...
              {ped.estado === 'listo_para_entrega' && (
                <>
                  <button onClick={notificarWhatsApp} disabled={accion === 'marcar-notificado'}
                  ...
                  <button onClick={() => call('marcar-entregado')} disabled={accion === 'marcar-entregado'}
...
export default function PedidosFabricacion() {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      // Extraer colegios únicos para el filtro
      const cols = [...new Map((data.pedidos || []).map(p => [p.id_colegio, p.nombre_colegio])).entries()]
        .map(([id, nombre]) => ({ id, nombre }))
      setColegios(cols)
    } catch {
      toast.error('Error cargando pedidos de fabricación')
    } finally {
      setLoading(false)
    }
  }, [filtroEstado])
  ...
  const pedidosFiltrados = pedidos.filter(p => {
    ...
    if (busqueda) {
      ...
      return (p.nombre_cliente || '').toLowerCase().includes(q) || String(p.id_pedido).includes(q)
    }
    return true
  })
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {colegios.length > 0 && (
          <select value={filtroColegio} onChange={e => setColegio(e.target.value)}
            className="pl-3 pr-8 py-2 border border-gray-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-amber-500">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {pedidosFiltrados.map(p => (
            <PedidoCard key={p.id_pedido} pedido={p} onRefresh={cargar} />
          ))}
        </div>
      )}
    </div>
